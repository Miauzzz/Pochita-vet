...
export async function createUsuarioCliente(profile, jwt, fullname) {
  ...
  const body = {
    data: {
      ...
      // copiamos fecha de creación desde User para que coincidan
  ...
  // En Strapi 5 los campos vienen planos en data (sin .attributes)
  const usuario = json.data; // { id, documentId, nombre, correo, tipo_usuario, ... }
  return { ...usuario };
}

/** Busca Usuario por correo en la collection "usuarios" */
export async function fetchUsuarioByEmail(email, jwt) {
  const url = `${STRAPI_URL}/api/usuarios?filters[correo][$eq]=${encodeURIComponent(
    email
  )}`;
  ...
  // Igual: campos planos directamente en entry
  return { ...entry }; // { id, documentId, nombre, correo, tipo_usuario, ... }
}
